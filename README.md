...
reason I started this library.  Around 40x faster than binascii.b2a_qp
on the bundled benchmark corpus.
...
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/benchmarks/b2a_qp-dark.svg">
  <img alt="Bar chart of quoted-printable encoding speed: zoomascii.b2a_qp 16,803 encodes per second, binascii.b2a_qp 425, pure-Python quopri 9.1" src="docs/benchmarks/b2a_qp.svg" width="720">
</picture>
...
The chart is generated by bin/bench_chart.py from bench.py's numbers
(best of three runs for each encoder, pinned to CPU 2 on an AMD Ryzen
7 PRO 7840U; Python 3.11.6, September 22, 2026).

For before/after optimization measurements, bin/bench_zoom.py can load
a saved build with `--baseline DIRECTORY` and alternate measurements
with the current build. Add `--per-file` to check each input separately.
See the script's docstring for build and comparison commands. This
benchmark preserves the corpus's original line endings.
...
The encoder classifies input 64 bytes at a time into a bitmask of the
bytes that need encoding, then jumps from one to the next, copying
the plain text between them in fixed-size chunks. On x86-64 the
classification uses SSE2 instructions; other architectures use a
portable version built from ordinary 64-bit arithmetic. No special
compiler flags or CPU features beyond baseline x86-64 are required.
